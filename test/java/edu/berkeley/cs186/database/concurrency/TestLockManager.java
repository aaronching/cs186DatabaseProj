...
public class TestLockManager {
  ...
  @Test
  @Category(StudentTestP3.class)
  public void testSameTransactionSimultaneousUpgrade() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    Thread thread1 = new Thread(new Runnable() {
      public void run() {
        /*Code to run goes inside here */
    ...
    thread2.start();
    thread2.join(100);
    assertTrue("Transaction 2 Thread should still be running", thread2.isAlive());
    assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED));

    thread3.start();
    thread3.join(100);
    assertTrue("Transaction 2 Second Thread should still be running", thread3.isAlive());
    assertFalse(lockMan.holdsLock("A", 2, LockManager.LockType.EXCLUSIVE));
    ...
    thread2.join(100);
    assertFalse("Transaction 2 Thread should have finished", thread2.isAlive());
    assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //T3 wakes, changes lock type, then wakes T2 which ultimately holds the lock

    thread3.join(100);
    assertFalse("Transaction 2 Second Thread should have finished", thread3.isAlive());
    assertTrue(lockMan.holdsLock("A", 2, LockManager.LockType.SHARED)); //still holds this lock
  ...
  @Test
  @Category(StudentTestP3.class)
  public void testSharedSimultaneousPromotionNotExclusive() throws InterruptedException{
    final LockManager lockMan = new LockManager();
    Thread thread1 = new Thread(new Runnable() {
      public void run() {
        /*Code to run goes inside here */
    ...
    thread4.join(100); //wait for thread 4 to finish
    assertTrue("Transaction 4 Thread should still be running", thread4.isAlive()); //T4 should be blocked
    ...
    lockMan.releaseLock("A", 4);
    assertFalse(lockMan.holdsLock("A", 4, LockManager.LockType.EXCLUSIVE));
  }

  @Test
  @Category(StudentTestP3.class)
  public void testUpgradeOnReleasePrioritized() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    Thread thread1 = new Thread(new Runnable() {
      public void run() {
        /*Code to run goes inside here */
    ...
    thread3.start();
    thread3.join(100);
    assertTrue("Transaction 3 Thread should still be running", thread3.isAlive());
    assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE));

    thread4.start();
    thread4.join(100);
    assertTrue("Transaction 1 Second Thread should still be running", thread4.isAlive());
    assertFalse(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE));

    lockMan.releaseLock("A", 2);

    thread3.join(100);
    assertTrue("Transaction 3 Thread should still be running", thread3.isAlive()); //lock upgrade is prioritized even though it is later in queue
    assertFalse(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE));

    thread4.join(100);
    assertFalse("Transaction 1 Second Thread should have finished", thread4.isAlive()); //lock upgrade can run first
    assertTrue(lockMan.holdsLock("A", 1, LockManager.LockType.EXCLUSIVE));
    ...
    assertTrue(lockMan.holdsLock("A", 3, LockManager.LockType.EXCLUSIVE));
  ...
  @Test
  @Category(StudentTestP3.class)
  public void testSharedRequestWaitsForDifferentExclusiveRequest() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    Thread thread1 = new Thread(new Runnable() {
      public void run() {
        /*Code to run goes inside here */
