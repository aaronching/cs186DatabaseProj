...
public class TestBPlusTree {
  ...
  public void testLookupKeyPageSplits() {
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < intLeafPageSize + 1; j++) {
        bp.insertKey(new IntDataType(i), new RecordID(i, j));
      }
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testIterateSamePage() {
    for (int i=0; i<10*intLeafPageSize; i++) {
      bp.insertKey(new IntDataType(i), new RecordID(0, i));
    }
    Iterator<RecordID> rids = bp.sortedScan();
    for (int i=0; i<10*intLeafPageSize; i++) {
      assertTrue(rids.hasNext());
  ...
  @Test
  @Category(StudentTest.class)
  public void testLocateLeafFindFirst() {
    for (int i=0; i<intLeafPageSize/2-1; i++) {
      bp.insertKey(new IntDataType(0), new RecordID(0, i));
    }
    for (int i=0; i<intLeafPageSize/2+1; i++) {
      bp.insertKey(new IntDataType(1), new RecordID(0, i));
    }
    LeafNode first = BPlusNode.getBPlusNode(bp, bp.getRootPageNum()).locateLeaf(new IntDataType(1), true);

    Iterator<RecordID> rids = first.scanFrom(new IntDataType(0));
    for (int i=0; i<intLeafPageSize/2-1; i++) {
      assertTrue("iteration "+i, rids.hasNext());
      RecordID rid = rids.next();
      assertEquals(i, rid.getSlotNumber());
    ...
    assertTrue(rids.hasNext());
    RecordID finalRID = rids.next();
    assertEquals(0, finalRID.getSlotNumber());
    assertFalse(rids.hasNext());

    rids = first.scanFrom(new IntDataType(1));
    assertTrue(rids.hasNext());
    rids.next();
    assertFalse(rids.hasNext());
  }
  ...
  @Test
  @Category(StudentTest.class)
  public void testLocateLeafNotFindFirst() {
    for (int i=0; i<intLeafPageSize/2-1; i++) {
      bp.insertKey(new IntDataType(0), new RecordID(0, i));
    }
    for (int i=0; i<intLeafPageSize/2+1; i++) {
    ...
    LeafNode first = BPlusNode.getBPlusNode(bp, bp.getRootPageNum()).locateLeaf(new IntDataType(1), false);

    Iterator<RecordID> rids = first.scanForKey(new IntDataType(0));
    assertFalse(rids.hasNext());
    ...
    for (int i=0; i<intLeafPageSize/2; i++) {
      assertTrue("iteration " + i, rids.hasNext());
      RecordID rid = rids.next();
      assertEquals(i+1, rid.getSlotNumber());
    }
    assertFalse(rids.hasNext());
  }
  ...
  @Test
  @Category(StudentTest.class)
  public void testInsertBetweenLeaves() {
    for (int i=0; i<2*intLeafPageSize; i+=2) {
      bp.insertKey(new IntDataType(i), new RecordID(0, i));
    }
    LeafNode first = BPlusNode.getBPlusNode(bp, bp.getRootPageNum()).locateLeaf(new IntDataType(399), false);

    Iterator<RecordID> rids = first.scan();
