...
public class QueryPlanCostsTest {
  ...
  public void testSeqScanOperatorCost() throws DatabaseException, QueryPlanException {
    List<String> intTableNames = new ArrayList<String>();
    intTableNames.add("int");
    ...
    int entries = transaction.getNumEntriesPerPage(tableName);
    ...
    QueryOperator qo  = new SequentialScanOperator(transaction, tableName);
    assertEquals(1, qo.estimateIOCost());

    List<DataType> values = new ArrayList<DataType>();
    values.add(new IntDataType(0));
    transaction.addRecord(tableName, values);
    qo  = new SequentialScanOperator(transaction, tableName);
    assertEquals(2, qo.estimateIOCost());
  }
}
