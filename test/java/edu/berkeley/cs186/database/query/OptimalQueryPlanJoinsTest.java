...
//  @Category(StudentTestP2.class)
  public void testTripleJoin() throws DatabaseException, QueryPlanException, IOException {
    File tempDir = tempFolder.newFolder("joinTest");
    Database d = new Database(tempDir.getAbsolutePath(), 4);
    Database.Transaction transaction = d.beginTransaction();
    Schema testSchema = TestUtils.createSchemaWithAllTypes();
    d.createTable(testSchema, "TempTableA");
    d.createTable(testSchema, "TempTableB");
    d.createTable(testSchema, "TempTableC");
    int numEntries = transaction.getNumEntriesPerPage("TempTableA");

    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableA", TestUtils.createRecordWithAllTypes().getValues());
    }

    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableB", TestUtils.createRecordWithAllTypes().getValues());
    }

    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableC", TestUtils.createRecordWithAllTypes().getValues());
    }

    transaction.queryAs("TempTableA", "t1");
    transaction.queryAs("TempTableB", "t2");
    transaction.queryAs("TempTableC", "t3");
    ...
    assertEquals(numEntries * numEntries * numEntries, count);
...
//  @Test
//  @Category(StudentTestP2.class)
  public void testTripleJoin2() throws DatabaseException, QueryPlanException, IOException {
    File tempDir = tempFolder.newFolder("joinTest");
    Database d = new Database(tempDir.getAbsolutePath(), 10);
    Database.Transaction transaction = d.beginTransaction();
    Schema testSchema = TestUtils.createSchemaWithAllTypes();
    d.createTable(testSchema, "TempTableA");
    d.createTable(testSchema, "TempTableB");
    ...
    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableA", TestUtils.createRecordWithAllTypesWithValue(1).getValues());
    }

    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableB", TestUtils.createRecordWithAllTypesWithValue(2).getValues());
    }

    for (int i = 0; i < numEntries; i++) {
      transaction.addRecord("TempTableC", TestUtils.createRecordWithAllTypesWithValue(3).getValues());
    }
