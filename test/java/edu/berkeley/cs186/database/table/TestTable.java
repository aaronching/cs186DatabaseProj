...
public class TestTable {
  ...
  @Test
  @Category(StudentTest.class)
  public void testFreePageReopen() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    for (int i=0; i<table.getNumEntriesPerPage()-1; i++) { //add full page - 1
      table.addRecord(input.getValues());
    }
    RecordID rid = table.addRecord(input.getValues()); //last entry
    assertEquals(1, rid.getPageNum());

    //add one more
    RecordID rid2 = table.addRecord(input.getValues());
    assertEquals(2, rid2.getPageNum());

    table.deleteRecord(rid);
    RecordID rid3 = table.addRecord(input.getValues());
    assertEquals(1, rid3.getPageNum()); //should have been added to first page
  }

  @Test
  @Category(StudentTest.class)
  public void testMultipleFreePagesReopen() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    for (int i=0; i<10*table.getNumEntriesPerPage(); i++) {
      table.addRecord(input.getValues());
    }
    for (int i=1; i<=10; i++) {
      table.deleteRecord(new RecordID(i, 0));
    }
    //readd to those slots
    for (int i=1; i<=10; i++) {
      RecordID rid = table.addRecord(input.getValues());
  ...
  @Test
  @Category(StudentTest.class)
  public void testUpdateFreePagesBackwards() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    for (int i=0; i<5*table.getNumEntriesPerPage(); i++) {
      table.addRecord(input.getValues());
    }
    table.deleteRecord(new RecordID(5, 0));
    table.deleteRecord(new RecordID(5, 1));
    ...
    for (int i=5; i>1; i--) {
      ...
      table.deleteRecord(new RecordID(i-1, 0));
      table.deleteRecord(new RecordID(i-1, 1));
    }
  ...
  @Test
  @Category(StudentTest.class)
  public void testIteratorGapBetweenPages() throws DatabaseException {
    int numEntries = table.getNumEntriesPerPage();
    RecordID[] toDelete = new RecordID[numEntries];
    for (int i=0; i<2*numEntries; i++) { //fill two pages
      Record input = TestUtils.createRecordWithAllTypesWithValue(i);
      RecordID rid = table.addRecord(input.getValues());
      if (i >= numEntries/2 && i < numEntries*3/2) {
        toDelete[i - numEntries/2] = rid;
      }
    }
    for (int i=0; i<toDelete.length; i++) { //delete the middle section (spans both pages)
      table.deleteRecord(toDelete[i]);
    }
    Iterator<Record> tableIter = table.iterator();
    for (int i=0; i<numEntries/2; i++) {
      assertTrue(tableIter.hasNext());
      Record r = tableIter.next();
      assertEquals(r.getValues().get(1).getInt(), i);
    }
    assertTrue(tableIter.hasNext());
    for (int i=3*numEntries/2; i<2*numEntries; i++) {
      assertTrue(tableIter.hasNext());
      Record r = tableIter.next();
      assertEquals(r.getValues().get(1).getInt(), i);
    }
