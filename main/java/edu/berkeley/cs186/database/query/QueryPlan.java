...
public class QueryPlan {
  ...
  private QueryOperator pushDownWheres(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    // TODO: implement me!
    QueryOperator returnSource = source;
    for (int i=0; i<whereColumnNames.size(); i++) {
      if (i != except) {
        try {
          String colName = source.checkSchemaForColumn(source.getOutputSchema(), whereColumnNames.get(i));
          returnSource = new WhereOperator(source, colName, whereOperators.get(i), whereDataTypes.get(i));
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    // TODO: implement me!
    int minCost = transaction.getNumDataPages(table);
    ...
    int minWhereIdx = -1;
    // TODO: implement me!
    for (int index : whereIndices) {
      IndexScanOperator iso = new IndexScanOperator(transaction, table, whereColumnNames.get(index), whereOperators.get(index), whereDataTypes.get(index));
      int indexScanCost = iso.estimateIOCost();
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    QueryOperator tryOp;
    int cost;
    for (JoinOperator.JoinType op : JoinOperator.JoinType.values()) {
      switch (op) {
        case SNLJ:
          tryOp = new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
          cost = tryOp.estimateIOCost();
          if (minCost == -1 || cost < minCost) {
            minOp = tryOp;
            minCost = cost;
          }
//          tryOp = new SNLJOperator(rightOp, leftOp, rightColumn, leftColumn, transaction);
//          cost = tryOp.estimateIOCost();
//          if (minCost == -1 || cost < minCost) {
//            minOp = tryOp;
//            minCost = cost;
//          }
          break;
        case PNLJ:
          tryOp = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
          cost = tryOp.estimateIOCost();
          if (minCost == -1 || cost < minCost) {
            minOp = tryOp;
            minCost = cost;
          }
//          tryOp = new PNLJOperator(rightOp, leftOp, rightColumn, leftColumn, transaction);
//          cost = tryOp.estimateIOCost();
//          if (minCost == -1 || cost < minCost) {
//            minOp = tryOp;
//            minCost = cost;
//          }
          break;
        case BNLJ:
          tryOp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
          cost = tryOp.estimateIOCost();
          if (minCost == -1 || cost < minCost) {
            minOp = tryOp;
            minCost = cost;
          }
//          tryOp = new BNLJOperator(rightOp, leftOp, rightColumn, leftColumn, transaction);
//          cost = tryOp.estimateIOCost();
//          if (minCost == -1 || cost < minCost) {
//            minOp = tryOp;
//            minCost = cost;
//          }
          break;
        case GRACEHASH:
          tryOp = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
          cost = tryOp.estimateIOCost();
          if (minCost == -1 || cost < minCost) {
            minOp = tryOp;
            minCost = cost;
          }
//          tryOp = new GraceHashOperator(rightOp, leftOp, rightColumn, leftColumn, transaction);
//          cost = tryOp.estimateIOCost();
//          if (minCost == -1 || cost < minCost) {
//            minOp = tryOp;
//            minCost = cost;
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    for (Map.Entry<Set, QueryOperator> entry : prevMap.entrySet()) {
      for (int i=0; i<joinTableNames.size(); i++) {
        ...
        String leftCol = joinLeftColumnNames.get(i);
        String rightCol = joinRightColumnNames.get(i);

        Set<String> checkRightTable = new HashSet<String>();
        checkRightTable.add(rightTable);
        if (!entry.getKey().contains(rightTable) && pass1Map.containsKey(checkRightTable)) {
          QueryOperator leftQO = entry.getValue();
          QueryOperator rightQO = pass1Map.get(checkRightTable);
          try {
            leftCol = leftQO.checkSchemaForColumn(leftQO.getOutputSchema(), leftCol);
            rightCol = rightQO.checkSchemaForColumn(rightQO.getOutputSchema(), rightCol);
          } catch (QueryPlanException e) {
            continue;
          }

          QueryOperator minCostJoin = minCostJoinType(leftQO, rightQO, leftCol, rightCol);

          if (leftQO.getType() != QueryOperator.OperatorType.JOIN) {
            QueryOperator testReverseOrder = minCostJoinType(rightQO, leftQO, rightCol, leftCol);
            if (testReverseOrder.estimateIOCost() < minCostJoin.estimateIOCost()) {
              minCostJoin = testReverseOrder;
            }
          }
