...
public class BPlusTree {
   ...
   */
    ...
    public BPlusIterator(LeafNode leaf, DataType key, boolean scan) {
      ...
      lookupKey = key;
      isScan = scan;
      currLeafIter = isScan ? currLeaf.scanFrom(lookupKey) : currLeaf.scanForKey(lookupKey);
    }
    ...
    public boolean hasNext() {
      ...
      if (currLeafIter.hasNext())
        ...
        if (currLeaf.getNextLeaf() == -1)
          return false;
        else {
          if (isScan)
            return true;
          else { //if not isScan, we are only looking for lookupKey
            //get the next leaf, scanForKey and see if the next leaf has lookupKey
            return ((LeafNode)BPlusNode.getBPlusNode(currLeaf.getTree(), currLeaf.getNextLeaf())).scanForKey(lookupKey).hasNext();
          }
        }
      }
    }
    ...
    public RecordID next() {
      ...
      if (hasNext()) {
        if (currLeafIter.hasNext())
          return currLeafIter.next();
        else {
          currLeaf = (LeafNode) BPlusNode.getBPlusNode(currLeaf.getTree(), currLeaf.getNextLeaf());
          currLeafIter = isScan ? currLeaf.scan() : currLeaf.scanForKey(lookupKey);
          return currLeafIter.next();
        }
      }
