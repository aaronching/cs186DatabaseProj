...
public class LockManager {
  ...
  private void handlePotentialDeadlock(Lock lock, long transNum, LockType lockType) {
    ...
    if (lock.getType() == LockType.EXCLUSIVE || (lock.getType() == LockType.SHARED && lockType == LockType.EXCLUSIVE)) {
      WaitsForGraph copy = waitsForGraph.copy();
      for (Long owner : lock.getOwners()) {
        if (owner != transNum) {
          if (copy.edgeCausesCycle(transNum, owner))
            throw new DeadlockException("Transaction " + transNum + " waiting for transaction " + owner + " would cause deadlock.");
          else
            copy.addEdge(transNum, owner);
        }
      }
      waitsForGraph = copy;
    }
