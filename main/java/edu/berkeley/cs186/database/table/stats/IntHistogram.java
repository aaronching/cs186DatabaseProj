...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int dataVal = value.getInt();
    int totalVals = getMaxValue() - getMinValue();
    switch (predicate) {
      case EQUALS:
        return ((float)1/getNumDistinct());
      case NOT_EQUALS:
        return 1-((float)1/getNumDistinct());
      case LESS_THAN:
        return ((float)dataVal - getMinValue())/totalVals;
      case LESS_THAN_EQUALS:
        return ((float)dataVal - getMinValue())/totalVals + ((float)1/getNumDistinct());
      case GREATER_THAN:
        return ((float)getMaxValue() - dataVal)/totalVals;
      case GREATER_THAN_EQUALS:
        return ((float)getMaxValue() - dataVal)/totalVals + ((float)1/getNumDistinct());
    }
